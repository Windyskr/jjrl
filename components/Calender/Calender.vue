<template>
	...
	<view class="qian_dao">
		<view class="header dis_f_sb_c">
			<view class="left dis_f_c_c" @click.stop="date_change('last')"><text>上月</text></view>
			<view class="center picker_btn">
				...
				<picker mode="date" fields="month" :value="date_info.ny" :start="startDate" :end="endDate"
					@change="date_change_fun"><text></text></picker>
			</view>
			<view class="right dis_f_c_c false" v-if="today.ny == date_info.ny"><text>下月</text></view>
			<view class="right dis_f_c_c" @click.stop="date_change('next')" v-else><text>下月</text></view>
		</view>
		<view class="section">
			<!-- 周，星期 -->
...
export default {
	...
	methods: {
		...
		// 判断是否在时间范围内
		dateInRang(id) {
			const idNum = this.dateToNum(id)
			const startNum = this.dateToNum(this.start)
			const endNum = this.dateToNum(this.end)
		...
		get_this_month_fun(list_days, n, y) {
			...
			for (var i = 1; i <= days; i++) {
				...
				let click = false;
				let isoday = false;
				if (this.dateInRang(nyr)) isoday = true;
				// 获取已经签到的列表
				let list = this.list;
				// 这天的数据
				let data = '';
	...
	watch: {
		...
		list(list) {
			...
			// this.get_this_month_fun(this.list_days, n, this.y);
		}
	},
	computed: {
		startDate() {
		...
		.days {
			.ul {
				...
				.day {
					...
					border-top: 1px solid #f0f0f0;
					border-left: 1px solid #f0f0f0;
					// background: url('@/static/geli.png') no-repeat;
					...
					&.true {
						// background: #007aff;
						font-size: 0;
						background: url('@/static/hesuan.png') no-repeat;
						color: #fff;
					}
				}
			}
